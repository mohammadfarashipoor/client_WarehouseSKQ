...
function ReportEmpolyees(props) {
  const { dataFilteredReports,
    fetchReportsHandle,
    filterReportForm,
    fetchHandleEmployees,
    formErrors,
    fetchEmployees,
    reportFilterFieldChange,
    submitFilterReport,
    isLoading,
    summary,
    pagination } = props;
  ...
  useEffect(() => {
    fetchHandleEmployees(null)
    fetchReportsHandle(currentPage)
  }, [currentPage])
  ...
  return (
    <TitleCard title="مدیریت گزارش ماهانه" styleClass="h-[80vh]">
      ...
      <div className="overflow-x-auto">
        {dataFilteredReports[0] ?
          ...
          : <NoContent className="mt-4" message="گزارش یافت نشد!" />}
      </div>
    </TitleCard>
  );
}
...
const mapStateToProps = (state) => ({
  ...
  pagination: state.reportEmployees.pagination,
